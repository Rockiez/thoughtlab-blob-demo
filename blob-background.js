...
// ============================================================
//  WebGL Raymarching Glass Bubble Background Effect
//  Reusable ES Module
// ============================================================
...
// ============================================================
//  Perspective Camera Class (Matching Original Setup)
// ============================================================
...
// ============================================================
//  Layout Mesh Class (Matching Original Bounds Math)
// ============================================================
...
// ============================================================
//  Reusable Background Module Class
// ============================================================
export class BlobBackground {
  constructor(options = {}) {
    ...
    if (!this.container) {
      console.error('BlobBackground: Container element not found.');
      return;
    }

    // Config Option Selector Classnames
    this.textSelector = options.textSelector || '[data-gl-text]';
    this.snapSelector = options.snapSelector || '[data-gl-snap]';
    this.sizeSelector = options.sizeSelector || '[data-gl-size]';
    this.scrollSnapTargetAttr = options.scrollSnapTargetAttr || 'data-gl-scroll-snap';
    this.hoverImgAttr = options.hoverImgAttr || 'data-gl-hover-img';
    this.scrollSnapImgAttr = options.scrollSnapImgAttr || 'data-gl-target-img';

    // Packaged Shader Parameters Options
    this.config = Object.assign({
      ...
      mouseDelta: 1.25,
      uSizeDefault: 0.14,
      cubemapPath: './static/cubemaps/01/',
    }, options.config || {});
    ...
    this.targetBlobSize = this.config.uSizeDefault;
    this.currentBlobSize = this.config.uSizeDefault;
  ...
  async init() {
    ...
    // Setup renderer element styles for absolute/fixed background overlay
    ...
    // Canvas textures for refraction text scan & hover showcase images
    ...
    this.mat = new THREE.ShaderMaterial({
      ...
      uniforms: {
        uTime: { value: 1.0 },
        uDistortionFrequency: { value: this.config.uDistortionFrequency },
        uDistortionStrength: { value: this.config.uDistortionStrength },
        uDisplacementFrequency: { value: this.config.uDisplacementFrequency },
        uDisplacementStrength: { value: this.config.uDisplacementStrength },
        uDisplacementScale: { value: this.config.uDisplacementScale },
        uFresnelOffset: { value: this.config.uFresnelOffset },
        uFresnelPower: { value: this.config.uFresnelPower },
        uFresnelMultiplier: { value: this.config.uFresnelMultiplier },
        uRefraction: { value: this.config.uRefraction },
        uRefractionColorShift: { value: this.config.uRefractionColorShift },
        tRender: { value: this.tRenderTexture },
        tMap: { value: cubemap },
        tRenderHover: { value: this.tRenderHoverTexture },
        uRenderHoverOpacity: { value: 0 },
        uColorMix1Opacity: { value: this.config.uColorMix1Opacity },
        uColorMix1Smooth: { value: this.config.uColorMix1Smooth },
        uHueShift: { value: this.config.uHueShift },
        uSaturation: { value: this.config.uSaturation },
        uRedSaturation: { value: this.config.uRedSaturation },
        uGreenSaturation: { value: this.config.uGreenSaturation },
        uBlueSaturation: { value: this.config.uBlueSaturation },
        uRedHue: { value: this.config.uRedHue },
        uGreenHue: { value: this.config.uGreenHue },
        uBlueHue: { value: this.config.uBlueHue },
        uMouse1: { value: new THREE.Vector2() },
        uMouse2: { value: new THREE.Vector2() },
        uResolution: { value: new THREE.Vector4() },
        uSize: { value: this.config.uSizeDefault },
        uOpacity: { value: 0 },
        uZoom: { value: 1.0 },
      }
    });
  ...
  // --- Dynamic DOM Refraction scanning & text wrap drawing ---
  updateRefractionCanvas() {
    ...
    const textElements = document.querySelectorAll(this.textSelector);
    textElements.forEach(el => {
      ...
      this.tRenderCtx.fillStyle = textColor;
  ...
  // --- Dynamic Hover Image canvas updating ---
  ...
  loadCubemap() {
    return new Promise((resolve) => {
      ...
      loader.setPath(this.config.cubemapPath);
      loader.load(
  ...
  fadeIn() {
    if (this.container && this.container.id) {
      const el = document.getElementById(this.container.id);
  ...
  // --- Native DOM events tracking (delegated globally) ---
  addEvents() {
  ...
  onTouchMove(e) {
    if (this.snapTarget || !e.touches.length) return;
    this.mouse.set(e.touches[0].clientX / window.innerWidth - 0.5, -(e.touches[0].clientY / window.innerHeight) + 0.5);
  }

  onMouseOver(e) {
    const card = e.target.closest(this.snapSelector);
    if (card && card !== this.hoveredCard) {
      this.hoveredCard = card;
      const imgPath = card.getAttribute(this.hoverImgAttr);
      let imgObj = null;
      ...
      if (imgPath) {
        if (!this.hoverImageCache[imgPath]) {
          ...
          imgObj = this.hoverImageCache[imgPath];
        }
      }
      ...
      if (imgObj) {
        ...
        this.targetBlobSize = this.config.BlobSizeHover * this.config.uSizeDefault;
      }
    }
  }
  ...
  checkScrollTriggers() {
    ...
    const sections = document.querySelectorAll(this.sizeSelector);
    let activeSize = this.config.uSizeDefault;
    ...
    sections.forEach(sec => {
      ...
      if (rect.top < window.innerHeight * 0.6 && rect.bottom > window.innerHeight * 0.4) {
        ...
        const snapSelector = sec.getAttribute(this.scrollSnapTargetAttr);
        if (snapSelector) {
          const targetEl = document.querySelector(snapSelector);
          if (targetEl) {
            newScrollSnapTarget = targetEl;
            const imgPath = targetEl.getAttribute(this.scrollSnapImgAttr);
            if (imgPath) {
              if (!this.hoverImageCache[imgPath]) {
                const img = new Image();
                img.src = imgPath;
    ...
    if (newScrollSnapTarget) {
      ...
      if (snapImgObj) {
        ...
        this.targetHoverOpacity = 1.0;
        this.targetBlobSize = this.config.BlobSizeHover * activeSize;
      }
    } else {
  ...
  animate() {
    ...
    this.mat.uniforms.uTime.value = this.time * this.config.uDisplacementSpeed;
  ...
  destroy() {
    ...
    if (this.tRenderTexture) this.tRenderTexture.dispose();
    if (this.tRenderHoverTexture) this.tRenderHoverTexture.dispose();
